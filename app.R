...
numeric.path.maker <- function(input, numeric.clonefreqs, current.clones)
{
  reactive({
...
server <- function(input, output, session)
{
  ...
  numeric.path.2 <- numeric.path.maker(input, numeric.clonefreqs.2, current.clones.2)
  ...
  output$path_plot <- path.plot.maker(input, numeric.path, recnum, "power", "cutoff")
  output$path_plot2 <- path.plot.maker(input, numeric.path.2, recnum.2, "power2", "cutoff2")
